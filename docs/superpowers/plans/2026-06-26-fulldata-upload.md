...
- **music_detail addressing — PARTIALLY understood; exact-bulk-fetch NOT reliable (exact-detail default OFF).** Reverse-engineered live: `index` is **per-category** (each `cat` restarts at 0); a `fetch()` of `music_detail.html?...&cat=N&index=M` returns real scores **iff** the server session's "current category" == N, set by **fetching `music.html?cat=N`**. Isolated probes worked (cat=0 list→detail → `達成率 0.6515`; cat=1 list→detail → real data). **BUT the deeper looped end-to-end test FAILED:** the detail session is **fragile** — category 0 (数字記号) needs `cat=`(empty) not `cat=0`, and once any detail request errors (`/error/` or `404`) it **poisons the session** so subsequent detail fetches `404`. A robust bulk pass was not achieved this way (the page is built for interactive single-song viewing). **Decision:** exact-detail fetching is an **experimental opt-in** (`spec.fetchDetails === true`, default OFF); the **default and reliable path is the 37-page rank sweep**. Exact achievement still comes from gsv (top-50) + the engine's Bayesian rank→achievement prior for the rest. The interleave logic is retained (and node-tested via mocked fetch) for a future, more careful solution (e.g. observing the real interactive click flow / a cleaner endpoint, or resetting the session after each error). `parseDetail` was also hardened from live HTML (nested `<th><div>level</div></th>` marker; unit-bearing counts like `"2 回"`); on a played song the live 最高ランク is a CSS clear-medal with no rank letter, so `buildPayload` uses the list rank.

  **Follow-up to investigate exact-detail WITHOUT brute-forcing:** open DevTools → Network on a fresh logged-in session, manually click one played song, and capture the exact request sequence/headers/cookies that yields scores; check whether errors are avoidable (always `cat=`(empty) for cat 0; re-fetch the list to reset the session after any error; slower throttle). Only then consider enabling `fetchDetails` by default.
