...
- **RESOLVED — music_detail addressing is a server-side session "current category".** Reverse-engineered live via fetch + DevTools/network probes (the false trail: `navigate()` deep-links return zeros or redirect to `/p/error/`, which is *not* how the bookmarklet works). Findings: `index` is **per-category** (each `cat` restarts at 0); a `fetch()` of `music_detail.html?...&cat=N&index=M` returns real scores **iff** the session's current category == N, where the current category is set by **fetching `music.html?cat=N`**; a cross-category detail fetch redirects to `/error/`. Confirmed: from a cat=0 page, fetching the cat=0 list then a cat=0 detail → `達成率 0.6515`; fetching the cat=1 list then a cat=1 detail → the cat=1 song's real data. **Fix:** `run()` is now an **interleaved per-category pass** — for each cat it fetches the list (sets the session) then fetches that category's selected details before moving on. Exact-detail is **re-enabled by default** (`spec.fetchDetails !== false`), capped by `detailCap` (default 120, consumed in category order). The interleave is covered by a node test (mocked fetch/timers) asserting list-before-detail order and an exact-achievement payload. `parseDetail` also hardened from live HTML (nested `<th><div>level</div></th>` marker; unit-bearing counts like `"2 回"`); on a played song the live 最高ランク is a CSS clear-medal with no rank letter, so `buildPayload` uses the list rank and the detail's exact achievement.
